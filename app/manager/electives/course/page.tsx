...
export default function ManagerCourseElectivesPage() {
  ...
  // Filter only course electives
  const courseElectives = electives?.filter((elective) => elective.type === "course") || []
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col gap-6">
        ...
        {/* Course Electives grid */}
        {courseElectives.length > 0 ? (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {courseElectives.map((elective) => (
                  ...
                  <div className="p-6 pt-2">
                    <div className="flex justify-between gap-2">
                      <Button asChild variant="outline" size="sm">
                        <Link href={`/manager/electives/course/${elective.id}`}>
                          <Eye className="mr-2 h-4 w-4" />
                          {t("common.view")}
                        </Link>
                      </Button>
                      <Button asChild variant="outline" size="sm">
                        <Link href={`/manager/electives/course/${elective.id}/edit`}>
